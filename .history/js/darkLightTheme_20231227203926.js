document.getElementById("switchButton").onclick = function () {
  document.getElementById("header").classList.toggle("dark");
  document.querySelector(".subscribe").classList.toggle("dark");
    document.querySelector(".about-us").classList.toggle("dark");
    document.getElementById("features").classList.toggle("dark");
    document.getElementById("services").classList.toggle("dark");
    document.getElementById("pricing").classList.toggle("dark");
    document.getElementById("petfood").classList.toggle("dark");
    document.getElementById("testimonial").classList.toggle("dark");
  
  // document.querySelectorAll(".li").classList.toggle("dark");
  
  };
  